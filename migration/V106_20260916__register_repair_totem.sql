-- main 의 V079_20260908__register_repair_totem.sql 를 magic-card 모델로 다시 쓴 것이다.
--
...
--                   'repair_totem' 은 게임 서버의 RepairTotemPrefabInitializer 가 서 있는 자리다.
--   element         V083 의 규칙대로 조합 {Build, Build, Nature} 에서 유일한 원소 카드인 Nature.
--   magic_cards     쓰지 않는다. 조합이 없어지고 읽는 코드도 없다. 원본의 조합 중복 검사도 같은 이유로 뺐다.
--   mana_cost       V084 의 규칙대로 build x2 + nature 의 mana_cost 합.
--   range           V084 의 규칙대로 시전 종류 game object 'build' 의 range.
--   prefab_elements RepairTotemPrefabInitializer.setElement(ElementType.NATURE) 에서 읽었다.
--   indicator       명시하지 않는다. V090 의 default 를 받고 V115 가 덮어쓴다.
--
--   hp = life_tree.hp x 1.0
--
-- radius 4.0, duration 6.0, mass 1000000.0 은 원본의 literal 그대로다. radius 는 V114 가 body 와
-- effect_radius 로 나누고, duration 은 V119 가 60.0 으로 올린다.
...
$$
    ...
    BEGIN
        ...
        FROM (VALUES ('life_tree', 'hp'),
                     ('build', 'mana_cost'), ('nature', 'mana_cost'),
        ...
        IF missing_reference IS NOT NULL THEN
            RAISE EXCEPTION 'repair_totem derives a value from %, which does not exist', missing_reference;
        END IF;
    END
$$;

INSERT INTO magics(name, element, cast_kind, prefab)
SELECT 'repair_totem', 'Nature', 'Summon', 'repair_totem'
WHERE NOT EXISTS (SELECT 1 FROM magics WHERE name = 'repair_totem');

UPDATE magics
SET element = 'Nature', cast_kind = 'Summon', prefab = 'repair_totem'
WHERE name = 'repair_totem'
  AND (element, cast_kind, prefab) IS DISTINCT FROM ('Nature', 'Summon', 'repair_totem');

INSERT INTO game_objects(name)
SELECT 'repair_totem'
WHERE NOT EXISTS (SELECT 1 FROM game_objects WHERE name = 'repair_totem');

INSERT INTO parameters(name)
SELECT required.name
FROM (VALUES ('hp'), ('radius'), ('duration'), ('mass'), ('mana_cost'), ('range')) AS required(name)
...
WITH cast_mana_cost AS (
    ...
    FROM (VALUES ('build'), ('build'), ('nature')) AS recipe(card_object)
     ...
     seed_values(parameter_name, value) AS (
         SELECT 'hp'::TEXT, (SELECT parameter_value.value
          ...
          WHERE game_object.name = 'life_tree' AND parameter.name = 'hp') * 1.0
         UNION ALL SELECT 'radius'::TEXT, 4.0::DOUBLE PRECISION
         UNION ALL SELECT 'duration'::TEXT, 6.0::DOUBLE PRECISION
...
$$
    ...
    BEGIN
        SELECT element, cast_kind, prefab INTO magic_row FROM magics WHERE name = 'repair_totem';

        IF magic_row IS NULL OR (magic_row.element, magic_row.cast_kind, magic_row.prefab)
            IS DISTINCT FROM ('Nature', 'Summon', 'repair_totem') THEN
            RAISE EXCEPTION 'repair_totem is not registered as (Nature, Summon, repair_totem)';
        END IF;
        ...
        -- 원본의 4개에 mana_cost 와 range 가 더해져 6개다.
        IF parameter_count <> 6 THEN
            RAISE EXCEPTION 'repair_totem has % parameter values, expected 6', parameter_count;
        END IF;
        ...
        FROM parameter_values parameter_value
                 JOIN game_objects game_object ON game_object.id = parameter_value.game_object_id
                 JOIN parameters parameter ON parameter.id = parameter_value.parameter_id
        WHERE game_object.name = 'life_tree' AND parameter.name = 'hp';

        SELECT parameter_value.value INTO own_hp
        FROM parameter_values parameter_value
                 JOIN game_objects game_object ON game_object.id = parameter_value.game_object_id
                 JOIN parameters parameter ON parameter.id = parameter_value.parameter_id
        WHERE game_object.name = 'repair_totem' AND parameter.name = 'hp';
        ...
        IF NOT EXISTS (SELECT 1 FROM prefab_elements WHERE prefab = 'repair_totem' AND element = 'Nature') THEN
            RAISE EXCEPTION 'prefab repair_totem has no Nature element row';
        END IF;
    END
$$;
