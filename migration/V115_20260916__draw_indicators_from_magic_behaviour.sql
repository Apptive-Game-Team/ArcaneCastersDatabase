-- main 의 V095_20260914__draw_indicators_from_magic_behaviour.sql 를 magic-card 로 옮긴 것이다.
--
-- 분류 C. 문서 내용은 원본 그대로이고, 두 줄만 고쳤다.
--
--   life_tree 와 healing_totem 의 치유 반경. 원본은 {"object": "life_tree", "parameter": "range"} 로
--   가리킨다. main 에서는 그 행에 1.5 가 들어 있어서 맞지만, magic-card 에서는 V084 가 마법 이름
--   game object 의 range 를 시전 사거리로 덮어썼다. V084 는 덮어쓰기 전에 옛 값을 effect_radius 로
--   옮겨 두었고 "healing_totem 과 life_tree 는 그 값을 Totem 의 치유 반경으로 넘긴다"고 적었다.
--   그래서 여기서는 두 마법의 두 번째 layer 가 effect_radius 를 가리킨다. 그대로 옮겼다면 치유 반경
--   자리에 build.range 인 6 이 그려졌을 것이다.
--
-- 나머지 마법의 문서는 한 글자도 바꾸지 않았다. 아래 assertion 이 크기 참조가 실제 parameter 행을
-- 찾는지 전부 검사하므로, 두 track 의 값 배치가 다른 곳이 더 있으면 이 파일이 실패해서 알려준다.
--
-- 이 파일은 V104~V111 이 등록한 마법 11개(wall_golem, shock_trap, repair_totem, grass_generator,
-- dragon_tower, firework_tower, titan_remnant, spirit_bomb, tidal_warhead, boulder_strike,
-- bomb_sprite)와 V114 가 만든 effect_radius 행에 기댄다. 번호가 그 뒤다.
--
-- 아래는 원본의 내용이고, 위에 적은 두 줄만 다르다.
-- ---------------------------------------------------------------------------
...
VALUES
    ...
    ('life_tree', '[{"shape": "circle", "origin": "target", "radius": {"object": "life_tree", "parameter": "radius"}}, {"shape": "circle", "origin": "target", "radius": {"object": "life_tree", "parameter": "effect_radius"}, "edgeWidth": 0.08}]'),
    ('healing_totem', '[{"shape": "circle", "origin": "target", "radius": {"object": "healing_totem", "parameter": "radius"}}, {"shape": "circle", "origin": "target", "radius": {"object": "healing_totem", "parameter": "effect_radius"}, "edgeWidth": 0.08}]'),
